...
import os, time, sys, logging, subprocess, random, shutil, socket, select, fcntl, threading, tempfile
...
def led_play(sequence, block, loop=1, daemon=True):
    def _run():
        loop_part = [s for s in sequence if s[1] is not None]
        final_part = [s for s in sequence if s[1] is None]
        for _ in range(loop):
            for state, duration in loop_part:
                set_led(state)
                time.sleep(duration)
        
        for state, _ in final_part:
            set_led(state)
    if block:
        _run()
    else:
